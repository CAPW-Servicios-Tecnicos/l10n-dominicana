...
class AccountMove(models.Model) :
    ...
    def _get_default_invoice_date(self) :
        return fields.Date.context_today(self) if self._context.get('default_type', 'entry') in self.get_purchase_types(
    ...
    bank_partner_id = fields.Many2one('res.partner', help='Technical field to get the domain on the bank',
    ...
    def update_lines_tax_exigibility(self) :
        if all(account.user_type_id.type not in {'payable', 'receivable'} for account in
               self.mapped('line_ids.account_id')) :
            ...
            base_lines_caba = self.line_ids.filtered(lambda x : any(tax.tax_exigibility == 'on_payment'
                                                                    or (tax.amount_type == 'group'
                                                                        and 'on_payment' in tax.mapped(
                        'children_tax_ids.tax_exigibility'))
    ...
    def _recompute_cash_rounding_lines(self) :
        ...
        def _compute_cash_rounding(self, total_balance, total_amount_currency) :
            ...
            if self.currency_id == self.company_id.currency_id :
                ...
                diff_amount_currency = self.invoice_cash_rounding_id.compute_difference(self.currency_id,
    ...
    def onchange(self, values, field_name, field_onchange) :
        ...
        return super(AccountMove, self.with_context(recursive_onchanges=False)).onchange(values, field_name,
                                                                                         field_onchange)
        ...
        if duplicated_moves :
            raise ValidationError(
                _('Duplicated vendor reference detected. You probably encoded twice the same vendor bill/credit note:\n%s') % "\n".join(
                    duplicated_moves.mapped(lambda m : "%(partner)s - %(ref)s - %(date)s" % {'ref' : m.ref,
        ...
        for move in self.filtered(lambda move : move.state == 'posted') :
            ...
            if move.date <= (lock_date or date.min) :
                if self.user_has_groups('account.group_account_manager') :
                    ...
                    message = _(
                        "You cannot add/modify entries prior to and inclusive of the lock date %s. Check the company settings or ask someone with the 'Adviser' role") % format_date(
                        self.env, lock_date)
    ...
    def _get_invoice_reference_euro_invoice(self) :
        ...
        reference = 'RF{} {}'.format(check_digits,
                                     " ".join(["".join(x) for x in zip_longest(*[iter(str(base))] * 4, fillvalue="")]))
        return reference
    ...
    def _get_invoice_reference_euro_partner(self) :
        ...
        reference = 'RF{} {}'.format(check_digits, " ".join(
            ["".join(x) for x in zip_longest(*[iter(partner_ref_nr)] * 4, fillvalue="")]))
        return reference
    ...
    def _get_invoice_computed_reference(self) :
        ...
        if self.journal_id.invoice_reference_type == 'none' :
            ...
            ref_function = getattr(self, '_get_invoice_reference_{}_{}'.format(self.journal_id.invoice_reference_model,
    ...
    def message_new(self, msg_dict, custom_values=None) :
        ...
        if partners :
            ...
            if is_internal_partner(partners[0]) :
                ...
                partners = [partner for partner in self._mail_find_partner_from_emails(body_mail_addresses) if
                            not is_internal_partner(partner)]
...
<<<<<<< HEAD
    ...
    def _convert_tags_for_cash_basis(self, tags) :
        ...
        """
        self.ensure_one()
        tax_multiplicator = (self.journal_id.type == 'sale' and -1 or 1) * (
                self.move_id.type in ('in_refund', 'out_refund') and -1 or 1)
...
class AccountPartialReconcile(models.Model) :
    ...
    def _get_tax_cash_basis_base_key(self, tax, move, line) :
        ...
        tax_rep_lines = tax.refund_repartition_line_ids if line.move_id.type in (
            'in_refund', 'out_refund') else tax.invoice_repartition_line_ids
        ...
        return (line.id, account_id.id, tax.id, line.tax_repartition_line_id.id, base_tags, line.currency_id.id,
                line.partner_id.id, line.move_id.type)
    ...
    def unlink(self) :
        ...
        for move in self.env['account.move'].search([('tax_cash_basis_rec_id', 'in', self._ids)]) :
            if move.date > (move.company_id.period_lock_date or date.min) :
                ...
                move._reverse_moves([{'date' : fields.Date.today(), 'ref' : _('Reversal of %s') % move.name}],
                                    cancel=True)
...
class AccountFullReconcile(models.Model) :
    ...
    def _prepare_exchange_diff_move(self, move_date, company) :
        ...
        if not company.income_currency_exchange_account_id.id :
            raise UserError(
                _("You should configure the 'Gain Exchange Rate Account' in the accounting settings, to manage automatically the booking of accounting entries related to differences between exchange rates."))
        if not company.expense_currency_exchange_account_id.id :
            raise UserError(
                _("You should configure the 'Loss Exchange Rate Account' in the accounting settings, to manage automatically the booking of accounting entries related to differences between exchange rates."))
